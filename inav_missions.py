...
def process_logs(df, spacing, max_wps, manual_alt=None):
    ...
    # Iteratively increase spacing if we exceed the waypoint limit
    while True:
        waypoints = []
        start_pos = None
        last_wp_pos = None
        ...
        for _, row in df.iterrows():
            try:
                # Handle GPS string: "Lat Lon" or "Lat, Lon"
                ...
                # Filter points outside USA or malformed
                ...
                # Safety check for sensor glitches (50km jump)
        ...
        current_spacing += 10 # Increase spacing and try again

@app.route('/', methods=['GET', 'POST'])
def index():
    if request.method == 'POST':
        file = request.files.get('file')
        mission_name = request.form.get('mission_name', '').strip() or "converted_mission"
        ...
        try:
            ...
            # Use octet-stream to prevent browser from forcing .xml extension
